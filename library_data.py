...
class ThumbnailsPreviewsThread(QThread):
    ...
    threads_pool = []
    def __init__(self, folder_data, _globals, current_index_centered_order=False):
        ...
        self.current_index_centered_order = current_index_centered_order
...
class LibraryData(BoardLibraryDataMixin, CommentingLibraryDataMixin, TaggingLibraryDataMixin):
    ...
    def list_interest_files(folder_path, deep_scan=False, all_allowed=None, check_windows_explorer_window=False):
        ...
        if all_allowed is None:
            all_allowed = not Settings.get("browse_images_only")
...
class BoardData():
    ...
    def __init__(self, folder_data):
        ...
        if ld.boardItemsTracking and LibraryData.NONE_FOLDER_NAME != folder_data.folder_label:
            self.crossboard_board_index = CrossboardData().acquire_board_index()
        else:
...
class FolderData():
    ...
    def do_rearrangement(self, insert_index):
        ...
        if Settings.get("autosave_on_reordering"):
...
class FileData():
    ...
    def is_animated_file(self):
        if self._is_animated_file is None:
            LD = LibraryData
            self._is_animated_file = any((
                    LD.is_gif_file(self.filepath)
                ,   LD.is_webp_file_animated(self.filepath)
                ,   self.is_animated_apng
            ))
        return self._is_animated_file

    # для страниц Board, Library, Viewer
    @property
    def is_animated_apng(self):
        if self._is_animated_apng is None:
            LD = LibraryData
