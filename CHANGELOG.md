...
- BOARDS (интерактивный коллаж)
    ...
    - появилось собственное цикличное слайдшоу с поочерёдными перелётами и зумом. После нажатия на клавишу `Пробел` запускается анимация перелистывания каждой картинки на доске. После повторного нажатия пробела анимация останавливается. Также цикл может пройти по заранее проставленным точкам, выставленным через `Ctrl+LMB`. Если имеется хоть одна выставленная точка, то анимация пойдёт по точкам.
        - ***Пока решил не создавать возможность на лету задавать колесом мыши величину скорости вплоть до полного разворота в противоположную сторону***
...
- BOARDS:
    ...
    - организована выгрузка тех изображений, которые очень далеко отстоят от камеры.
        - **Изначально хотелось начинать выгружать по количеству загруженных и вдобавок делать это рандомно, но пока сужу по расстоянию между центром камеры и центром картинки. Возможно, поздней надо будет сделать настройку для отключения этой фичи**
...
- BOARDS
    - (20 янв 26) добавил autoscroll для страницы доски
        ...
        - **может и для страницы вьювера имеет смысл сделать подобное, но пока не буду делать**
        - (23 янв 26) сделал плавным изменение масштаба во время автоскролла
            ...
            - **заметил, что скорость масштабирования начинает увеличиваться из-за скорости перемещения. Этот эффект можно скомпенсировать поздней, пока понаблюдаю насколько сильно он меня будет бесить**
            - **кстати, может следует избавиться от таймера для зума и перенести его код в таймер для autoscroll, а то получается слишком много таймеров богу таймеров**
- ОБЩЕЕ
...
- BOARDS
    - (26 янв 26) недавно сделал автоскролл, чтобы можно было просматривать доску без геймпада
        ...
        - для WASD изменились эффекты для клавиш-модификаторов: Shift теперь позволяет шпарить со скоростью 100x, а Alt замедляет её, и в итоге скорость получается 20x
            - **хотя мне кажется, что в будущем лучше сделать переключение скоростей через НАжатие клавиш, а не ЗАжатие, как это происходит сейчас**
    ...
    - (21 янв 26)-(29 янв 26) WATERFALL - новая страница в программе!
        - она будет совмещать столбцы больших превьюшек из правого столбца страницы Library и давать возможность модального просмотра каждой картинки при клике на превьюшке
            - (30 янв 26) доступен модальный просмотр
                ...
                - **справка F1 не обновлялась для этой страницы всё ещё, так что перед релизом надо обязательно сделать**
                - **контекстное меню не задавалось, и ни одного пункта для страницы viewer не перенесено**
                - **есть идея сделать выход из модального режима по ЛКМ за пределами изображения**
                - **модальный вьювер загружает масштаб и положение картинки, которые у ней были бы при просмотре на странице viewer, но не сохраняет эти параметры при выходе из модального режима**
...
- WATERFALL
    ...
    - (12 мар 26) для превьюшек в немодальном режиме реализовано перелистывание перемещением курсором мыши слева направо и обратно
        - **закэшированные хайрезные оригиналы никуда не выгружаются, позже надо будет прописать чистку памяти**
- LIBRARY
...
- ОБЩЕЕ
    ...
    - (28 фев 26) обновлён и дополнен перевод на русский
        - **названия страниц в настройках пока не переводятся, потому что я ещё пока не определился как назвать Waterfall**
        - **также в названиях настроек повторяются названия раздела настроек, в будущем от этого надо избавиться**
...
- BOARDS
    ...
    - (25 фев 26) UX: скраббинг курсором мыши для анимационных файлов на доске
        ...
        - **нюанс: нет учёта поворота айтема вообще, для анимационных файлов это выливается в то, что ползунок не учитывает ориентацию**
    ...
    - (27 фев 26) в настройках можно отключить загрузку board-плагинов на старте приложения
        ...
        - (4 мар 26) перестали включаться плагины, если они запущены из командной строки, исправил это
            - но исправил я это предварительной загрузкой вообще всех плагинов, а это в будущем может дать тормоза при загрузке другого плагина, **надо об этом помнить**
    ...
    - (27 фев 26) наконец-то Ctrl+Z для отмены вращения, перемещения и масштабирования айтемов
        ...
        - **история действий не обрабатывает специальные случаи, когда переносится айтем-фрейм с айтемами в нём**
        ...
        - **на данный момент история изменений не записывается для других команд, которые влияют на положение, ориентацию и масштаб айтемов**
    ...
    - (4 мар 26) с геймпада теперь можно листать кадры видимых в окне анимационные айтемов (а заодно айтемы папок и групп)
        ...
        - **UX: можно синхронизировать прокрутку айтемов, которые видно, но оставлю это на будущее**
        - **UX: вертикальную линию-оверлей как у Cursor Scrubber создавать не стал**
        - **UX: можно и скорость прокрутки увеличивать, если кнопка зажата секунду**
    - (6 мар 26) СЕНСАЦИЯ: прогрессивная развёртка айтемов-изображений на доске
        ...
        - ради этой фичи у айтемов появился атрубут visible, потому что код генерации превьюшек вызывает board-методы, и из-за этого айтемы мелькают в области ориджина перед тем как попасть на своё место
            - **по идее надо переписать это всё через сигналы, но GIL позволяет такому существовать**
    ...
    - (16 мар 26) Aling & Distribute
        ...
        - **пока не всё настроено, но кое что уже работает**
