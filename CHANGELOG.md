...
BOARDS
  - система плагинов
      ...
      - активация плагина
      - перезадание функции отрисовки на примере отладочного плагина Example Plugin
      ...
      - отладочный плагин Example Plugin доработан так, чтобы поддерживать паннинг и зум и при этом сохранять прежнее управление
      - активация досок плагинов перемещена в подменю контекстного меню
      - создан плагин-шаблон `template_plugin.py` для удобства разработки следующих плагинов
  ...
  - написана основа плагина Watch Tower, который добавляет на виртуальную доску изображения из списка папок, задаваемого заранее в текстовом файле `watch_tower.data` в папке `user_data` 
      - каждая папка - отдельный столбец
  ...
  - сохранение и загрузка файла доски
      - во время дебага `Globals.DEBUG = True` можно открыть недавно сохрнанившийся файл, для этого нужно нажать сначала `Ctrl+S`, а потом `Ctrl+O`
      - поправил код так, чтобы анимированные айтемы скролились после загрузки из файла доски
      - переписал генератор reg-файла, после активации которого .board-файлы можно открывать в Image Viewer двойным кликом (сам Image Viewer пока не понимает, что это файл доски и его надо окрыть как доску)
