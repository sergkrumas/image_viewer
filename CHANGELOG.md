...
- если файл не поддерживается, но есть в списке, то в сообщение оторбражается имя файла
- BOARDS
    - новый тип айтема для доски - айтемы-фреймы, в основном несут чисто вспомогательную функцию, визуально обводят группу айтемов
        ...
        - загловки этих айтемов-фреймов **ПОКА** менять невозможно
    ...
    - теперь можно нырнуть и в анимированные изображения, а на доске будут кадры из анимированного изоражения
        - **выгрузка контента и освобождение памяти не реализованы**
    - около начала координат каждой доски теперь выводится инфа о доске
    ...
    - пришла идея сделать табы для открытых досок, чтобы не приходилось лезть в библиотеку для переключения между ними каждый раз. Число табов будет расти по мере ныряния в новые доски
        - **потом**
...
- BOARDS
    ...
    - пофиксил баг: после клика по картинке без перетаскивания её не работало закрытие окна через Esc (и многие другие вспомогательные функции, повешенные на эту клавишу)
...
- исправлен баг: в упрощённом режиме приложение остаётся висеть в памяти, если окно закрылось через всплывающую миинатюру окна или через контекстное меню панели задач Windows
...
- теперь из формы редактирования тегов для изображения можно удалить тег и всю его информацию
        - при удалении тега удаляется инфа из базы данных в оперативной памяти, удаляются файлы с информацией с диска, удаляется виртуальная папка тега из библиотекиы
...
- доступен поиск потерянных картинок в специальной форме; форма отображается через клавишу Q
    - алгоритм
        - все изображения, которые помечены тегом, имеют комментарии или добавлены в избранное проверяются на существование по путям на диске. Если файл отстутсвует по прописанному пути, то он добавляется на поиск, вдобавок из записей запоминается размер в байтах и значение md5-хэша
        ...
        - исправляются записи в памяти и на диске, обновляются виртуальные папки на стринце библиотеки
    - проверка
    ...
    - **для дальнейших доработок**
        ...
        - поиск файла в папке очень тупой и неоптимизированный, например, для каждой потерянной записи в каждой папке для поиска будет заново просчитываться размер и md5
...
- BOARDS
    ...
    - исправлег баг: при скейле относительно противоположного угла как раз именно этот самый противоположный угол съезжал по координатам. Особенно это было заметно, если был задан поворот
