    ...
    def make_copy(self, canvas, folder_data, file_data=None, copy_dependent_data=True):
        ...
        if self.file_data is not None:
            if self.file_data.folder_data is folder_data:
                ...
                copied_item.file_data = file_data
            else:
...
class BoardMixin(BoardTextEditItemMixin):
    ...
    def board_serial_to_object_attributes(self, obj, obj_attrs_list, promises, fd=None, id_to_filedata=None, all_items=None):
        for attr_name, attr_type, attr_data in obj_attrs_list:
            ...
            elif attr_type == 'BoardUserDefinedPlacesList':
                user_defined_places = []
                for user_point in attr_data:
                    point_tuple = user_point[0]
                    region_width = user_point[1]
                    region_height = user_point[2]
                    user_defined_places.append((QPointF(*point_tuple), region_width, region_height))
                obj.user_defined_places = user_defined_places
                continue
    ...
    def board_serialize_board_data(self, fod):
        ...
        self.board_object_attributes_to_serial(board, board_attributes,
                                                            exclude=('items_list', 'user_defined_places'))
        ...
        user_defined_places_serialized = []
        for pos, region_width, region_height in board.user_defined_places:
            user_defined_places_serialized.append(((pos.x(), pos.y()), region_width, region_height))
        board_attributes.append(('user_defined_places', 'BoardUserDefinedPlacesList', user_defined_places_serialized))
    ...
    def board_gather_children_boards(self):
        out = dict()
        for fd in self.CrossboardData().children_boards_folder_data:
    ...
    def board_draw_main_default(self, painter, event):
        ...
        self.board_draw_user_defined_places(painter, cf)
    ...
    def board_delete_selected_board_items(self):
        ...
        if self.selected_items or selected_links:
            ...
            if selected_links:
                stat += '\n' + _("links: {}").format(len(selected_links))
    ...
    def board_get_removed_items_group(self, folder_data):
        ...
        self.board_set_tracking_data(gi, folder_data)
        ...
        self.CrossboardData().add_board_folder_data(item_folder_data)
    ...
    def board_add_item_group(self, move_selection_to_group=True, virtual_allowed=False, item_position=None):
        ...
        gi = BoardItem(BoardItem.types.ITEM_GROUP)
        gi.item_folder_data = item_folder_data
        self.board_set_tracking_data(gi, current_folder_data)
    ...
    def board_add_item_note(self):
        ...
        self.board_set_tracking_data(ni, current_folder_data)
    ...
    def board_add_item_folder(self, folder_path=None):
        ...
        if folder_path:
            with self.show_longtime_process_ongoing(self, _("Loading folder to the board")):
                ...
                self.board_set_tracking_data(fi, _fd)
    ...
    def board_do_add_item_frame(self, fod, content_bounding_rect):
        ...
        self.board_set_tracking_data(bi, fod)
        fod.board.items_list.append(bi)
    ...
    def board_snapping_define_targets(self):
        ...
        class SnappingTarget():
            ...
            def get_target_deactivation_dist(self, snap_pos, cursor_pos):
                dist = QVector2D()
                if self.type == SnappingTarget.types.POINT:
                ...
                for bi in cf.board.items_list:
                    if bi not in self.selected_items:
                        ...
                        if sa_br.intersects(board_mapped_viewport_rect):
                            ...
                            # у каждого айтема, и не брать превые две точки, но у каждого айтема разная ширина и высота,
                            # да и айтемы могут переместить на другое место, повернуть или отмасштабировать,
    ...
    def board_right_click_selection_releaseEvent(self, event):
        ...
        if all(i.type != i.types.ITEM_LINK for i in self.item_magazin):
            item_magazin = self.board_filter_magazin(self.item_magazin)
    ...
    def board_control_v(self):
        ...
        if self.CROSSBOARD.activated:
            self.show_center_label(_("Disabled temporarily for crossboard"), error=True)
            return
    ...
    def board_fly_over(self, user_call=False):
        ...
        if not self.fly_pairs:
            ...
            use_user_defined_places = bool(cf.board.user_defined_places)

            if use_user_defined_places:
                menu = RoundedQMenu()
                ...
                if a is None:
                    ...
                    use_user_defined_places = False

            if use_user_defined_places:
                for point, region_width, region_height in cf.board.user_defined_places:
    ...
    def board_get_all_crossboard_fods(self):
        ...
        children_fods = self.CrossboardData().children_boards_folder_data
    ...
    def board_create_boards_hierarchy_graph_link(self, ni1, ni2):
        li = BoardItem(BoardItem.types.ITEM_LINK)
        li.from_item = ni1
        li.to_item = ni2
    ...
    def board_build_boards_hierarchy_graph(self, origin, fod, level=0, parent_node=None):
        ...
        if parent_node is not None:
            self.board_create_boards_hierarchy_graph_link(parent_node, ni)
        for item in fod.board.items_list:
    ...
    def board_copy_hierarchically_dependent_data(self, metadata, original_item, copied_item):
        ...
        # восстановление ссылок между айтемами и досками, привязанных к ним
        for level in levels:
            for fod_dict in metadata[level]:
                fod = fod_dict['fod']
        ...
        # TODO: копировать линки во вложенных досках, надо объединить списки линков со всех досок и убедиться, что все линки в нём уникальные
            ...
            # а что будет, если линка будет между айтемами, которые зависимо находятся каждый в своей айтеме-ноде, причём последние были выделены пользователем для копирования?
                # получается, линки надо копировать только под конец копирования всего как промисы, а здесь делать эти промисы
        ...
        for item in items_to_replace:
            if item._fod_to_move is not None:
                ...
                if item.file_data is not None:
                    ...
                    # TODO (9 май 26): также надо поставить защиту от копирования виртуальных FileData,
                    # которые применяются для досок с кадрами анимированных айтемов.
                    # И хоть такие доски не должны отображаться на кроссдоске, защита всё равно должна быть.
